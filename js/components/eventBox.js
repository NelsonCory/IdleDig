class EventBox extends Phaser.GameObjects.Container{
    constructor(config){
        ...
        this.eventName = this.scene.add.text(this.eventBack.x,Math.floor(this.eventBack.y - this.eventBack.height*0.37),this.event.eventName,  {fontFamily: 'Font_Fantasy', fontSize: 27, color: 'black'}); // 0.18
        ...
        this.eventText = this.scene.add.text(this.eventBack.x,Math.floor(this.eventBack.y + this.eventBack.height * 0.09),this.event.eventText, { fontFamily: 'Everyday_Standard', fontSize: 10, color: 'black',wordWrap: { width: this.eventBack.width - 70}, align: "center"  }); //0.55
